...
prefs-title = 言語と表示
prefs-language = 言語
prefs-language-note = 変わるのはインターフェースだけです。人名、地名、覚書はつねにそれぞれの言語と文字で表示されます。
prefs-theme = 表示
prefs-apply = 適用
prefs-reviewed = 校閲済み
...
tree-lede-focused = 祖先{ $ancestors }名、子孫{ $descendants }名、配偶者・伴侶{ $spouses }名、各方向{ $depth }世代。古い世代ほど下に並びます。線の濃さは関係の確度で、薄い線は記録が確信を持てていない主張です。
tree-lede-whole = 家系図に含まれるすべての人。古い世代が下、新しい世代が上です。線の濃さは関係の確度を表します。
...
admin-recent-note = { $path } に記録された{ $total }件のうち直近の{ $shown }件。履歴はアーカイブの中ではなく隣に置かれます。アーカイブは複製され、送られ、公開されるものであり、誰が何を直したかは家系図を管理する人についての事実であって、そこに記された家族についての事実ではないからです。
...
accounts-branch-hint = このアカウントが編集できる範囲を、指定した人々とその子孫、配偶者に限ります。読める範囲は制限しません。それは各記録の公開範囲が決めることであり、二つは意図的に切り離してあります。
...
tree-width-notice = この表示は幅{ $width }ピクセルです。各世代が一行で、もっとも広い行がこの幅を決めます。幅1500ピクセルの画面なら{ $screens }画面分の横スクロールにあたります。一人を中心とした表示なら、その人の周囲の数十名だけを示し、どのカードからでも中心を移せます。
...
tree-contradicts-detail = ある人が自分自身の祖先として記録されているか、同じ直系上の二人が夫婦として記録されています。どのように行を並べても成り立たないため、その関係は世代の番号付けから除いてあり、一部の行が誤っている可能性があります。管理画面から検証を実行して原因をお探しください。
...
tree-click-hint = カードをクリックすると、その人の記録が横の欄に開きます。欄内の「ここを家系図の中心にする」で表示の起点が移ります。
...
home-what-title = これが家族にもたらすもの
home-what-archive-title = アーカイブ全体を一か所に
home-what-archive-body = 家系図と資料と写真が一緒に収まります。婚姻証書の複写は誰かのメールの中ではなく婚姻そのものにぶら下がり、写真には写っている人の名前がつきます。
home-what-together-title = 複数の親族、それぞれの役割
home-what-together-body = 三十年分の覚書を持つ叔母と、綴りを一つ直したいだけの従兄弟に、同じ権限は要りません。親族はそれぞれの役割で招かれ、変更はすべて誰がいつ行ったかを残します。
home-what-privacy-title = 公開範囲は一人ずつ決める
home-what-privacy-body = 存命の親族は家族には見えて訪問者には見えない、その曽祖母は誰にでも開かれている、という設定ができます。判断は一人ずつ行うもので、家系図全体に一度だけ決めるものではありません。
home-what-languages-title = 十一の言語
home-what-languages-body = 親族はそれぞれの言語でこのサイトを読めます。中東欧の半分で戸籍簿が書かれた言語であるロシア語も含みます。名前はラテン文字転写とならべて元の文字のまま残ります。サイトを動かすために一つの文字体系へ均す必要はありません。
home-what-export-title = アーカイブはあなたのものです
home-what-export-body = いつでも全体を一つのファイルとして書き出せます。人物、関係、資料、写真がまとめて含まれます。いつか離れると決めたときも、アーカイブはそのまま手元に残ります。
home-in-this-tree = 家族がこれまでに記録したもの
home-showcase-title = この家系図がすでに名前と日付以上を語っているところ
home-showcase-note = ここに挙がるのは、サイトができることの一覧ではなく、実際にここに記録されている内容です。
...
record-verbatim-note = どの変換器も解釈できなかったため、記録にあったとおりに残してあります。もう一方の選択肢は捨てることでした。
...
convert-lede = 既存の家系図を GEDCOM ファイルから移せます。GEDCOM はたいていの系譜ソフトが書き出す形式です。手元に残せるアーカイブが返ってきます。ここには何も保存されず、このサイトがすでに表示している家系図もそのままです。
...
convert-lang-hint = en、fr、ja のような表示です。一つの場所は複数の言語で名前を持てます。ここではお手元のファイルの地名が何語で書かれているかを指定します。
convert-what-you-get = 取り込みが加えるもの
convert-what-you-get-1 = どの事実にも、あとから調整できる確からしさがつきます。疑いを捨てずに書き留められるということです。日付は形を保ちます。1500年頃、1430年以前、1920年から1925年のあいだは三つの別の主張のままで、誰も日付として読めなかった表現は一字一句そのまま残ります。職業は始まりと終わりのある期間になります。場所はそれぞれ独立した記録になるので、国が変わった町はその履歴を保ちます。
convert-no-way-back = .ged ファイルへの書き戻しは提供しません。あの形式には、事実の確からしさ、家族の外の関係、職業の長さ、誰にも特定できなかった日付を置く場所がありません。戻る道はそれらを黙って落とします。代わりに、あなたのアーカイブは一つのファイルとして丸ごと書き出せます。
...
convert-skipped-note = これらの項目には、移せる中身がありませんでした。飲み込まずに列挙しています。何が後に残されたかを正確に知れることが、信用できる取り込みとそうでない取り込みの違いです。
...
convert-download-note = ここには十五分だけ置かれ、その後破棄されますので、今のうちにダウンロードしてください。この一つのファイルが家系図の全体です。安全な場所に保管してください。
...
completeness-intro = 何が記録され、何がまだ空欄かを示します。ここに誤りはありません。空欄は記録が育ちうる場所であって、うまくいかなかった箇所ではありません。
