...
radar-section-help = Три прочтения того, что содержит эта запись, каждая ось от 0 до 100. Каждое число вычисляется из фактов, указанных рядом, по правилам, записанным в документации приложения; ничего не сохраняется и не угадывается, а ось, из которой нечего прочитать, остаётся пустой, а не получает средний балл. Метки и полосы показывают, насколько надёжно каждое число: закрашенная точка — практически несомненно, точка в кольце — хорошо подтверждено, кольцо — правдоподобно, пунктирное кольцо — предположение; чем длиннее полоса, тем меньше уверенность.
