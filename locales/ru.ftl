...
prefs-language = Язык
prefs-language-note = Это меняет только интерфейс. Имена, места и заметки всегда показываются на своём языке и в своей графике.
...
tree-lede-focused = { $ancestors ->
    ...
    }, по { $depth } поколений в каждую сторону. Старшие внизу. Прозрачность линии — это уверенность в родстве: бледная линия означает утверждение, в котором запись не уверена.
tree-lede-whole = Все люди в древе. Старшие внизу, младшие наверху. Прозрачность линии — это уверенность в родстве.
...
admin-recent-note = Последние { $shown } из { $total ->
    ...
    }, из { $path }. Журнал лежит рядом с архивом, а не внутри него: архив копируют, пересылают и публикуют, а кто что исправил — факт о людях, ведущих древо, а не о семье внутри него.
...
accounts-branch-hint = Ограничивает то, что эта учётная запись может править, названными людьми, их потомками и супругами. Это не ограничивает то, что она может читать — этим ведает видимость каждой записи, и эти две вещи намеренно разделены.
...
tree-width-notice = Этот вид шириной { $width } пикселей. Каждое поколение — один ряд, и самый широкий из них задаёт эту ширину; на экране в 1500 пикселей это { $screens ->
        [one] один экран
        [few] { $screens } экрана
        [many] { $screens } экранов
       *[other] { $screens } экранов
    } прокрутки по горизонтали. Вид вокруг одного человека показывает вместо этого несколько десятков человек рядом с ним, и любая карточка перецентрирует его.
...
tree-contradicts-detail = Кто-то записан своим собственным предком, либо двое на одной линии происхождения записаны как пара. Никакая расстановка рядов этого не удовлетворит, поэтому спорное родство исключено из нумерации поколений и часть рядов может быть неверна. Запустите проверку из панели управления, чтобы найти его.
...
tree-click-hint = Нажмите любую карточку, чтобы открыть запись этого человека на панели; «Центрировать древо здесь» на панели переносит корень вида.
...
home-what-title = Что это даёт семье
home-what-archive-title = Одно место для всего архива
home-what-archive-body = Древо, документы и фотографии лежат вместе. Скан свидетельства о браке висит на самом браке, а не в чьей-то почте, а фотография называет тех, кто на ней.
home-what-together-title = Несколько родственников, разные роли
home-what-together-body = Тётя с тридцатью годами записей и двоюродный брат, который хочет лишь исправить опечатку, не нуждаются в одинаковых правах. Каждого родственника приглашают со своей ролью, и каждое изменение сохраняет, кто и когда его внёс.
home-what-privacy-title = Видимость решается по каждому человеку
home-what-privacy-body = Живущий родственник может быть виден семье и невиден посетителям, а его прабабушка открыта всем. Выбор делается для каждого человека, а не один раз для всего древа.
home-what-languages-title = Одиннадцать языков
home-what-languages-body = Родственники читают сайт на своём языке — в том числе по-русски, а именно на русском велись метрические книги и акты гражданского состояния половины Центральной и Восточной Европы. Имя остаётся в своей графике рядом с транслитерацией; ничего не нужно сводить к одному алфавиту, чтобы сайт работал.
home-what-export-title = Архив остаётся вашим
home-what-export-body = Выгрузите всё одним файлом когда угодно — людей, связи, документы и фотографии вместе. Если однажды решите уйти, вы уходите с целым архивом.
home-in-this-tree = Что семья записала на сегодня
home-showcase-title = Там, где это древо уже говорит больше, чем имена и даты
home-showcase-note = Каждый из этих пунктов взят из того, что здесь действительно записано, а не из списка того, что сайт умеет.
...
record-verbatim-note = Сохранено ровно так, как значилось в записи, потому что ни один преобразователь не смог это истолковать. Другим выходом было бы выбросить.
...
convert-lede = Перенесите существующее древо из файла GEDCOM — это выгрузка, которую делает большинство генеалогических программ. Вы получаете обратно архив, который остаётся у вас. Здесь ничего не хранится, а древо, которое сайт уже показывает, остаётся ровно таким, каким было.
...
convert-lang-hint = Метка вроде en, fr или ru. Место может хранить своё название на нескольких языках; это говорит, на каком языке написаны названия в вашем файле.
convert-what-you-get = Что добавляет импорт
convert-what-you-get-1 = Каждый факт получает степень уверенности, которую можно потом поправить, так что сомнение записывается, а не выбрасывается. Даты сохраняют свою форму: около 1500, до 1430 и между 1920 и 1925 остаются тремя разными утверждениями, а формулировка, которую никто не смог прочитать как дату, сохраняется слово в слово. Занятие становится отрезком времени с началом и концом. Каждое место становится отдельной записью, так что город, сменивший государство, сохраняет эту историю.
convert-no-way-back = Обратная запись в файл .ged не предлагается. Тому формату негде хранить степень уверенности в факте, связь вне семьи, длительность занятия или дату, которую никто не смог установить, — обратный путь тихо их потеряет. Ваш архив выгружается вместо этого целиком, одним файлом.
...
convert-skipped-note = В этих записях не было ничего, что можно перенести. Они перечислены, а не проглочены: знать, что именно осталось позади, — это и есть разница между импортом, которому можно доверять, и импортом, которому нельзя.
...
convert-download-note = Хранится здесь пятнадцать минут, затем удаляется, поэтому скачайте сейчас. Этот один файл и есть всё древо; держите его в надёжном месте.
...
completeness-intro = Что записано, а что пока пусто. Ничто здесь не ошибка: пустая строка — это место, где запись может вырасти, а не то, что пошло не так.
