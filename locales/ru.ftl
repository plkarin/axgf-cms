...
life-nothing-recorded = Для этого человека не записано ни события, ни занятия, ни места. Для преобразованного файла это обычное дело: GEDCOM несёт то, что кто-то записал, а большинство записей — имя и дата.
life-add-first = Записать первое событие
...
union-role-spouse = супруг
union-role-partner = партнёр
union-role-husband = муж
union-role-wife = жена
...
diff-section-other = другое поле
diff-section-avatar = Портретное фото
diff-section-partners = Партнёры
diff-section-ai = Помощь ИИ
diff-section-birth = Рождение
diff-section-civil-status = Гражданское состояние
diff-section-documents = Прикреплённые документы
diff-section-extensions = Данные приложения
diff-section-children = Дети
diff-section-union = Союз
diff-section-date = Дата
diff-section-participants = Участники
diff-section-from = Связь от
diff-section-to = Связь с
diff-section-valid-from = Действует с
diff-section-valid-until = Действует до
diff-section-visibility = Видимость
diff-section-employer = Работодатель
diff-section-title-normalized = Нормализованная должность
diff-section-conflicts = Противоречащие источники
diff-section-dna = ДНК
diff-section-document = Документ
diff-section-language = Язык
diff-section-place = Место
diff-section-repository = Архив
diff-section-script = Письменность
diff-section-coordinates = Координаты
diff-section-country-history = История государств
diff-section-identifiers = Идентификаторы
diff-section-names = Названия
diff-section-file = Файл
diff-section-linked-to = Прикреплён к
diff-section-ocr = Распознанный текст
diff-section-format-version = Версия формата
...
record-union-duplicate = Одна пара, больше одной записи.
record-union-duplicate-detail = В пакете хранятся отдельные записи семьи для этих двух людей. Это дефект данных, а не второй союз.
record-union-duplicate-action = Объединить дубликаты
record-union-duplicate-confirm = Устранить дубликаты во всём пакете? Каждая пара, которую библиотека может объединить, будет объединена; об остальных будет сообщено.
dedup-pair-merged = Указанная пара теперь одна запись.
dedup-pair-refused = Указанная пара не объединена: библиотека отказала и оставила её на рассмотрение человеку.
...
family-lineage = Происхождение
links-relation = Вид связи
occupations-position = Должность
field-link-relation = Вид связи
field-link-relation-hint = Одна из связей, названных в AXGF 1.1. Название выше сохраняет слова записи.
field-occupation-position = Должность
field-occupation-position-hint = Должность в рамках занятия: Директор, если занятие — Учитель.
error-delete-changed-title = Изменено после вашего просмотра
error-delete-changed-detail = Эту запись сохранили снова после того, как была показана страница, с которой вы её удаляете; теперь это версия { $version }. Ничего не удалено. Посмотрите на неё в нынешнем виде, прежде чем решать снова.
error-delete-changed-look = Посмотреть снова
documents-files = Файлы, приложенные здесь
documents-files-help = Измените сведения о файле или удалите сам файл. Удаление убирает документ и его байты из архива для всех, к кому он приложен; чтобы отцепить его только от этого человека, очистите его строку выше.
documents-edit-details = Изменить сведения
documents-delete = Удалить этот файл

## Charts

radar-section = Диаграммы по записи
radar-section-help = Три прочтения того, что содержит эта запись, каждая ось от 0 до 100. Каждое число вычисляется из фактов, указанных рядом, по правилам, записанным в документации приложения; ничего не сохраняется и не угадывается, а ось, из которой нечего прочитать, остаётся пустой, а не получает средний балл.
radar-physique = Телосложение
radar-mind = Темперамент и ум
radar-vitality = Здоровье и жизненные силы
radar-axis-stature = Рост
radar-axis-build = Сложение
radar-axis-lean-mass = Безжировая масса
radar-axis-posture = Осанка
radar-axis-gait = Походка
radar-axis-dentition = Зубы
radar-axis-openness = Открытость
radar-axis-conscientiousness = Добросовестность
radar-axis-extraversion = Экстраверсия
radar-axis-agreeableness = Доброжелательность
radar-axis-stability = Эмоциональная устойчивость
radar-axis-cognition = Когнитивные функции
radar-axis-circulation = Кровообращение
radar-axis-breathing = Дыхание
radar-axis-metabolism = Обмен веществ
radar-axis-illness = Отсутствие болезней
radar-axis-senses = Органы чувств
radar-axis-rest = Сон и настроение
radar-folded-open = Показать эту диаграмму
radar-folded-why = Этот человек записан как живой. Портрет темперамента живого человека остаётся свёрнутым, пока его не попросит показать тот, кому разрешено его читать.
radar-empty = В этой записи пока нет ничего, из чего можно построить эту диаграмму.
radar-table-caption = { $chart }: каждая ось, её балл и то, из чего он прочитан
radar-col-axis = Ось
radar-col-score = Балл
radar-col-from = Прочитано из
radar-no-score = нет балла
radar-from-none = ничего
record-link-outgoing = от этого человека
record-link-incoming = к этому человеку
# Эксплуатационное предупреждение в панели администратора.
health-attention = Требует внимания:
health-standing-token = Аварийный токен администратора всё ещё задан. Он входит в обход любой учётной записи и любых прав — для этого он и нужен в день, когда войти не может никто, но не для того, чтобы остаться, когда кто-то уже может. Уберите его из файла окружения службы и перезапустите её.
health-bundle-invalid = { $errors ->
        [one] Семейные данные больше не проходят проверку: { $errors } ошибка, перечислена ниже в разделе «Проверить».
        [few] Семейные данные больше не проходят проверку: { $errors } ошибки, перечислены ниже в разделе «Проверить».
        [many] Семейные данные больше не проходят проверку: { $errors } ошибок, перечислены ниже в разделе «Проверить».
       *[other] Семейные данные больше не проходят проверку: { $errors } ошибки, перечислены ниже в разделе «Проверить».
    }
health-disk-unknown = Свободное место на этой машине прочитать не удалось, поэтому ничто здесь не может обещать, что следующее сохранение поместится.
health-disk-no-room-to-save = Сохранение невозможно: свободно { $free }, а для перестроения этого архива нужно { $need }. Ничего не потеряно, и ничего нельзя изменить, пока не появится место.
health-disk-critical = Диск свободен на { $percent } % — осталось { $free }. Сохранение вскоре перестанет работать.
health-disk-low = Диск свободен на { $percent } % — осталось { $free }. Стоит заняться, пока не стало срочным.
health-backup-unconfigured = Ничего не резервируется. Один вышедший из строя диск забрал бы с собой все записи.
health-backup-never = Каталог резервных копий задан, но в него ни разу не была записана копия.
health-backup-stale = { $days ->
        [one] Самой свежей копии { $days } день. Её следует создавать ежедневно.
        [few] Самой свежей копии { $days } дня. Её следует создавать ежедневно.
        [many] Самой свежей копии { $days } дней. Её следует создавать ежедневно.
       *[other] Самой свежей копии { $days } дня. Её следует создавать ежедневно.
    }
health-cache-missing = { $missing ->
        [one] { $missing } из { $declared } вложенных файлов отсутствует в кеше и не может быть скачан, пока следующее сохранение его не восстановит.
        [few] { $missing } из { $declared } вложенных файлов отсутствуют в кеше и не могут быть скачаны, пока следующее сохранение их не восстановит.
