...
prefs-title = 语言与外观
prefs-language = 语言
prefs-language-note = 这只改变界面。人名、地名和笔记始终以其原有的语言和文字显示。
prefs-theme = 外观
prefs-apply = 应用
prefs-reviewed = 已校订
...
tree-lede-focused = 祖先{ $ancestors }人、后代{ $descendants }人、配偶或伴侣{ $spouses }人，每个方向{ $depth }代。年长者在下。连线的深浅表示这段关系的可信度，淡线是记录本身也不确定的说法。
tree-lede-whole = 家谱中的每一个人。年长者在下，年幼者在上。连线的深浅表示这段关系的可信度。
...
admin-recent-note = 来自 { $path } 的{ $total }条已记录改动中的最近{ $shown }条。日志放在档案旁边而不是里面：档案会被复制、寄送和公开，而谁改了什么是关于维护家谱的人的事实，不是关于其中家族的事实。
...
accounts-branch-hint = 把这个账户可编辑的范围限制为这些人、他们的后代和配偶。它不限制可阅读的范围——那由每条记录的可见范围决定，两者是有意分开的。
...
tree-width-notice = 此视图宽{ $width }像素。每一代占一行，其中最宽的一行决定了这个宽度——在1500像素的屏幕上相当于{ $screens }屏的横向滚动。以一个人为中心的视图只显示他周围的数十人，而且任一张卡片都能重新定中心。
...
tree-contradicts-detail = 有人被记为自己的祖先，或者同一条直系上的两个人被记为夫妻。任何行的排布都无法满足这一点，因此那段关系被排除在代数编号之外，部分行可能有误。请从管理面板运行校验来找出它。
...
tree-click-hint = 点击任意卡片，即可在侧栏打开那个人的记录；侧栏中的「以此为家谱中心」会移动视图的起点。
...
home-what-title = 这对一个家族有什么用
home-what-archive-title = 整份档案放在一处
home-what-archive-body = 家谱、文献和照片放在一起。一张结婚证书的扫描件挂在婚姻本身上，而不是躺在谁的邮箱里；一张照片则写明照片上的人是谁。
home-what-together-title = 多位亲属，不同角色
home-what-together-body = 一位记了三十年笔记的姑姑，和一位只想改一个字的表弟，不需要相同的权限。每位亲属都以各自的角色受邀，每一次改动都记下是谁在什么时候做的。
home-what-privacy-title = 可见范围逐人决定
home-what-privacy-body = 在世的亲属可以对家族可见、对访客不可见，而他的曾祖母则对任何人开放。这个选择是为每个人分别作出的，不是为整棵家谱一次作出的。
home-what-languages-title = 十一种语言
home-what-languages-body = 亲属用自己的语言阅读本站——包括俄语，中东欧半数地区的户籍簿正是用俄语记录的。姓名保留其原有文字，旁边并列拉丁转写；本站无需把一切压平到同一套字母才能运作。
home-what-export-title = 档案始终是您的
home-what-export-body = 随时把全部内容导出为一个文件——人物、关系、文献和照片都在里面。若有一天您决定离开，带走的是完整的档案。
home-in-this-tree = 这个家族到目前为止记录了什么
home-showcase-title = 这棵家谱已经说出的，不止是姓名和日期
home-showcase-note = 以下每一条都来自这里实际记录的内容，而不是本站功能的清单。
...
record-verbatim-note = 完全按记录原样保留，因为没有任何转换器能解读它。另一种做法就是把它丢掉。
...
convert-lede = 从 GEDCOM 文件迁入已有的家谱——GEDCOM 是大多数家谱软件导出的格式。您会得到一份可以自己保存的档案。这里不保存任何内容，本站已经展示的家谱也原封不动。
...
convert-lang-hint = 类似 en、fr 或 zh 的标记。一个地点可以用多种语言保存自己的名称；这里说明您文件中的地名是用哪种语言写的。
convert-what-you-get = 导入会补上什么
convert-what-you-get-1 = 每个事实都会获得一个日后可调整的可信度，于是疑问可以写下来而不是被丢掉。日期保持原有形态：约1500年、1430年之前、1920年至1925年之间，仍是三种不同的说法；没有人能读作日期的措辞，会一字不改地保留。职业成为有起止的一段时间。每个地点成为独立的条目，因此改换过国家的城镇会保留这段历史。
convert-no-way-back = 本站不提供写回 .ged 文件。那个格式没有地方放置一个事实的可信程度、家族之外的关系、一份职业持续多久，或是谁都无法确定的日期——回程会悄悄丢掉它们。作为替代，您的档案可以整份导出为一个文件。
...
convert-skipped-note = 这些条目里没有可以迁移的内容。它们被逐条列出而不是被吞掉：确切知道什么留在了后面，正是可信的导入与不可信的导入之间的差别。
...
convert-download-note = 在此保留十五分钟，随后丢弃，请现在就下载。这一个文件就是整棵家谱；请妥善保存。
...
completeness-intro = 哪些已经记录，哪些仍是空的。这里没有一处是错误：空着的一行是记录可以生长的地方，不是出了岔子的地方。
